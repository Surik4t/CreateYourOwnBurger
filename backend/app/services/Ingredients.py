...
async def add_ingredient(new_ingredient: IngredientModel):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Could not create an ingredient: {e}")
...
async def update_ingredient(ingredient_id, updated_ingredient: IngredientModel):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error updating ingredient: {e}")
...
async def remove_ingredient(ingredient_id):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error deleting ingredient: {e}")
